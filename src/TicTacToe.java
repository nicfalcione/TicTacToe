...
public class TicTacToe {
    ...
    /**
     * Enumeration used to represent taken coordinates on the board as tokens.
     * ("x" and "o" in TICTACTOE game)
     */
    enum token {
     ...
     * tracked here.
     * 
     * @param s Scanner from main to read user command line inputs
     * @throws InterruptedException thrown if the Thread.sleep(delay) calls fail
     */
    public static void playGame(Scanner s) throws InterruptedException {
        ...
        // This loop is where the user moves are read and made and the CPU AI
        // moves are
        // made.
        while (true) {
            ...
            if (CPUAI.winStrategy() != CPU_STRATEGY_NOT_NEEDED) {
                ...
                placeToken(CPUMove, cpu);
            } else if (CPUAI.defensiveStrategy() != CPU_STRATEGY_NOT_NEEDED
                    && !difficulty.equals(NOVICE)) {
                CPUMove = CPUAI.defensiveStrategy();
                placeToken(CPUMove, cpu);
            } else if (CPUAI.aggresiveStrategy() != CPU_STRATEGY_NOT_NEEDED
    ...
    public static String flipCoin() {
        ...
        try {
            ...
            // Generates a number between 0.0 and 1.0, which simulates flipping
            // a coin
            if (Math.random() < 0.5) {
    ...
    public static void wonCoinToss() {
        System.out.println(
                "Congratulations, you've won the coin toss!\nYou get to go first!");
    }
     ...
     * 
     * @param choice coordinate to be placed
     * @param t      token to be placed
     */
    public static void placeToken(int choice, token t) {
        board[GameState.convertRow(choice)][GameState.convertCol(choice)] = t
                .getVal();
    }
     ...
     * 
     * @param s Scanner from main to read from command line
     * @return player token
     */
